...
Wenn ``r config$sched$name`` einen Job ausführt, setzt es eine Reihe von Umgebungsvariablen für den Job. Mit einer dieser Variablen können wir überprüfen, von welchem Verzeichnis aus unser Job-Skript eingereicht wurde. Die Variable `SLURM_SUBMIT_DIR` wird auf das Verzeichnis gesetzt, von dem aus unser Job eingereicht wurde. Ändern Sie Ihren Job mit Hilfe der Variable `SLURM_SUBMIT_DIR` so, dass er das Verzeichnis ausgibt, von dem aus der Job übermittelt wurde.
