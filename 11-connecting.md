...
Wenn Sie dazu aufgefordert werden, geben Sie ein sicheres Passwort ein, das die [obigen Überlegungen berücksichtigt](#uberlegungen-zu-ssh-schlusselpasswortern). Beachten Sie, dass sich das Terminal nicht zu ändern scheint, während Sie das Passwort eingeben: Das ist zu Ihrer Sicherheit so gewollt. Sie werden aufgefordert, es noch einmal einzugeben, also machen Sie sich nicht zu viele Gedanken über Tippfehler.
