...
Verwenden Sie `pwd` (print working directory), um den aktuellen Verzeichnispfad zu drucken:

```bash
[yourUsername@login1 ~] pwd
```

Sie können `ls` (list) ausführen, um den Inhalt des Verzeichnisses aufzulisten, obwohl es möglich ist, dass nichts angezeigt wird (wenn keine Dateien bereitgestellt wurden). Um sicher zu gehen, benutzen Sie das `-a` Flag, um auch versteckte Dateien anzuzeigen.
...
Sie können auch die verfügbaren Dateisysteme mit `df` (disk freespace )untersuchen, um Festplattenspeicher anzuzeigen. Das Flag `-h` zeigt die Größen in einem menschenfreundlichen Format an, d.h. GB statt B. Das type Flag `-T` zeigt, welche Art von Dateisystem jede Ressource ist.
